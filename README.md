...
含まれています。AppSpec は、アプリ ID、OIDC binding、Takos リソースの permission
grants、公開 MCP エンドポイント、Cloudflare Worker ホストバンドルの build
設定を宣言します。

`.takosumi.yml` AppSpec はホスト Worker を `worker` component としてモデル化し、
サンドボックスホストワーカーの build、`/gui` ダッシュボード、`/mcp` 公開 surface
を宣言します。Cloudflare Containers、Durable Object binding、managed
`SESSION_INDEX` KV namespace、生成された `SANDBOX_HOST_AUTH_TOKEN`、
`MCP_AUTH_TOKEN`、`PUBLISHED_MCP_AUTH_TOKEN`、`APP_SESSION_SECRET` サービス env
要件は provider-specific metadata / bindings として記録されます。
...
これら 3 つのトークンはそれぞれ異なる値でなければなりません。AppSpec は
...
が AppSpec の readiness パス、`/healthz` は bootstrap-safe な liveness probe
です。
