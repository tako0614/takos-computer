...
`listen.oidc.from: operator.identity.oidc` 由来の `OIDC_*` env
...
worker の `publish.http`、gateway component の descriptor-owned `spec.listeners`
...
`.takosumi.build.yml` 側の profile で `deno task build:all` を実 行し、prepared
source に含める runtime file を生成します。bundle を更新する作業 でも
`deno task build:all` を実行してから commit します。
...
サンドボックス内 CLI 用に Takos API キーを `TAKOS_TOKEN` / `TAKOS_API_URL`
として消費でき、`TAKOS_TRUST_ROUTED_GUI_API=1` を設定して dispatch ルーティング
された GUI / API 呼び出しが Takos の `X-Takos-Internal-Marker`
ヘッダーを使えるようにします。Cloudflare Containers Durable Object binding
`SANDBOX_CONTAINER` (`SandboxSessionContainer`) と、 `apps/sandbox/Dockerfile`
コンテナイメージ metadata は Wrangler fallback 側の deploy config
で扱います。`/readyz` は strict readiness path、`/healthz` は bootstrap-safe な
liveness probe です。
...
prepared source contains `dist/sandbox-host.js` as the runtime file used by
Takos deploy. This repo keeps that generated Worker bundle in git as a
maintenance policy. ソースは `packages/computer-hosts/src/`
配下とダッシュボード生成 GUI アセットが真実の源です。`dist/` は lint / format
の対象外で、ソース変更後は `deno task build:all`
を実行し、`deno task check:dist` でバンドルのドリフトを検出します。
