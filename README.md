...
checkout から `bun run build:all` を実行して prepared source を作り、Installer
API へ `source.kind=prepared` として渡します。prepared source は
...
Takosumi Source dry-run は repository metadata と operator PlatformService
inventory から InstallPlan を作ります。`/gui` ダッシュボード、`/mcp`、health /
