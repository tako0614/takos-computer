...
Takos ディストリビューションに同梱される 1st-party App として Takosumi
上で動作します。サンドボックス / MCP 表面自体は Takos 固有の結合を持たず、任意の
MCP 対応エージェントホストから利用できます。

## 主要技術

- Bun tooling + Deno-based sandbox container
- Hono 4
- MCP SDK
- Zod
...
Takosumi Install では `APP_AUTH_REQUIRED=1` と
`listen.oidc.path: identity.primary.oidc` 由来の `OIDC_*` env
が注入されます。GUI は `state` + `nonce` + PKCE S256 の OIDC authorization code
flow を使い、`/gui` scoped の HttpOnly session cookie を発行します。install
直後の `/gui/api/auth/launch?launch_token=...` は Takosumi Accounts の
launch-token consume endpoint を呼び、one-time token を local GUI session
に交換してから `/gui` に戻します。
...
- [Deno](https://deno.land/) >= 2.0 — sandbox container runtime
...
| `PUBLISHED_MCP_AUTH_TOKEN`       | _(none)_    | 公開 `/mcp` エンドポイントを保護する bearer トークン。operator deploy config または secret 管理で供給します                                  |
...
| `INSTALL_LAUNCH_INSTALLATION_ID` | _(none)_    | launch token consume 対象 Installation ID                                                                                                    |
...
リポジトリには Takosumi installer 用 `.takosumi.yml` が含まれています。build
service / CI は Git checkout から prepared source を作り、Installer API へ
`source.kind=prepared` として渡します。AppSpec は、アプリ ID、worker
component、gateway component、worker `spec.entrypoint: dist/sandbox-host.js`、
worker の `http` output、gateway component の descriptor-owned `spec.listeners`
/ `spec.routes`、OIDC listen source を宣言します。prepared source は
`dist/sandbox-host.js` を runtime file として含めます。この repository では
maintenance policy として generated bundle を git に 保持していますが、Takosumi
packaging の前提ではありません。build service / CI は AppSpec ではなく
`.takosumi.build.yml` 側の profile で `bun run build:all` を実 行し、prepared
source に含める runtime file を生成します。bundle を更新する作業 でも
`bun run build:all` を実行してから commit します。

`.takosumi.yml` AppSpec はホスト Worker を `worker` component としてモデル化し、
HTTP material を `web.http` として publish します。公開側の ingress は `gateway`
component の descriptor-owned `spec.listeners` / `spec.routes` として宣言し、
`/gui` ダッシュボード、`/mcp`、health / readiness、session API は Worker の内部
route として扱います。 Cloudflare Containers、Durable Object
binding、`SESSION_INDEX` KV namespace、
`SANDBOX_HOST_AUTH_TOKEN`、`MCP_AUTH_TOKEN`、`PUBLISHED_MCP_AUTH_TOKEN`、
`APP_SESSION_SECRET` などの runtime env / bindings は operator の deploy config
または Wrangler fallback で供給します。
...
これら 3 つのトークンはそれぞれ異なる値でなければなりません。runtime は
サンドボックス内 command-line tools 用に Takos API キーを `TAKOS_TOKEN` /
`TAKOS_API_URL` として消費でき、`TAKOS_TRUST_ROUTED_GUI_API=1` を設定して
dispatch ルーティング された GUI / API 呼び出しが Takos の
`X-Takos-Internal-Marker` ヘッダーを使えるようにします。Cloudflare Containers
Durable Object binding `SANDBOX_CONTAINER` (`SandboxSessionContainer`) と、
`apps/sandbox/Dockerfile` コンテナイメージ metadata は Wrangler fallback 側の
deploy config で扱います。`/readyz` は strict readiness path、`/healthz` は
bootstrap-safe な liveness probe です。
...
prepared source contains `dist/sandbox-host.js` as the runtime file used by the
Takosumi prepared-source apply path for this bundled app. This repo keeps that
generated Worker bundle in git as a maintenance policy. ソースは
`packages/computer-hosts/src/` 配下とダッシュボード生成 GUI
アセットが真実の源です。`dist/` は lint / format の対象外で、ソース変更後は
`bun run build:all` を実行し、`bun run check:dist`
でバンドルのドリフトを検出します。
