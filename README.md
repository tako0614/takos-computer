...
maintenance policy として generated bundle を git に保持していますが、Takosumi
packaging の前提ではありません。bundle を更新する作業でも `bun run build:all`
を実行してから commit します。
...
prepared source contains `dist/sandbox-host.js` as the runtime file consumed by
the generated-root/source-archive apply path. This repo keeps that generated
Worker bundle in git as a maintenance policy. ソースは
