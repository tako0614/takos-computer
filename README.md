...
`listen.oidc.path: takosumi.identity.oidc` 由来の `OIDC_*` env
...
Takosumi v1 は plain OpenTofu module と Git metadata を入力にします。build service / CI は Git
checkout から `bun run build:all` を実行して immutable runtime artifact を作り、
OpenTofu module がその artifact ref / digest を参照します。runtime artifact は
`dist/sandbox-host.js` を runtime file として含めます。`dist/` は local / CI の
生成物であり、この repository には commit しません。hosted install では Git
release または CI artifact の URL と sha256 を OpenTofu 変数として渡します。

Takosumi の `plan` Run は Source snapshot、OpenTofu plan artifact、policy decision を記録します。credentials は
ProviderConnection / CredentialRecipe / ProviderBinding / policy で fail-closed に解決されます。`/gui` ダッシュボード、`/mcp`、health /
readiness、session API は Worker の内部 route として扱います。Cloudflare
Containers、Durable Object binding、`SESSION_INDEX` KV namespace、
...
から ProviderBinding と Capsule Output projection として供給します。
...
`apps/sandbox/Dockerfile` コンテナイメージ metadata は generated root が参照する
...
コンテナエンドポイントに転送します。operator が低レベルの Wrangler デバッグを
行う場合も、Takosumi の OpenTofu Capsule install flow が生成した binding /
env contract と同じ material だけを使います。Install / production 導線は
Takosumi の plan/apply Run ledger を正本にし、README の手順として raw
Wrangler deploy を公開しません。

### Worker バンドル

local/self-host apply では `bun run build:all` で `dist/sandbox-host.js` を生成してから
OpenTofu を実行します。hosted install では Git release / CI artifact を参照し、
`worker_bundle_url` と `worker_bundle_sha256` のような module input で digest pin
します。ソースは
`packages/computer-hosts/src/` 配下とダッシュボード生成 GUI
アセットが真実の源です。`dist/` は lint / format の対象外で、`bun run check:dist`
は bundle を一時ディレクトリへ生成できることを検証します。
