...
`namespace:operator.identity.oidc` listen 由来の `OIDC_*` env
が注入されます。GUI は `state` + `nonce` + PKCE S256 の OIDC authorization code
flow を使い、`/gui` scoped の HttpOnly session cookie を 発行します。install
...
リポジトリには Takosumi installer による Git URL install 用 `.takosumi.yml` が
含まれています。AppSpec は、アプリ ID、worker component、worker
`spec.entrypoint: dist/sandbox-host.js`、routes、OIDC listen source
を宣言します。 build command は AppSpec ではなく `.takosumi.build.yml` にあり、
`deno task build:all` で prepared source 内の `dist/sandbox-host.js`
を生成します。

`.takosumi.yml` AppSpec はホスト Worker を `worker` component としてモデル化し、
`/gui` ダッシュボード、`/mcp`、health / readiness、session API routes
を宣言します。 Cloudflare Containers、Durable Object binding、`SESSION_INDEX` KV
namespace、
